...
def arp_spoofing(mac,ip):
    ...
    packet = ARP(op=2,hwdst="ff:ff:ff:ff:ff:ff",psrc=ip,hwsrc=mac,pdst=target_ip)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2:
        ...
        if argvlist[1] == "-help":
            print("[ ARP sniffer and spoof program ]")
            print("Format :")
            print("1) python arp.py -l -a")
            print("2) python arp.py -l <filter_ip_address>")
            print("3) python arp.py <query_ip_address>")
            print("4) python arp.py <fake_mac_address> <target_ip_address>")
    elif len(sys.argv) == 3:
        argvlist = sys.argv
