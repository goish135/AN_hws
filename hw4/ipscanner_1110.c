...
int main (int argc, char **argv)
{
  ...
  {	  
	  ...
	  */
	  ...
	  // strcpy (target, "140.117.172.87"); // change! > .1~.254
	  for(int i=1;i<=254;i++)
	  {
	  ...
	  */
	  ...
	  // Sequence Number (16 bits): starts at 1 > update: starting from 1 
	  ...
	  for (;;) {
		...
		//printf("code: %d\n",recv_icmphdr->icmp_code);
            ...
            printf("PING %s (data size = 10, id = 0x2657 ,seq = %d ,timeout = 10000 ms)\n",target,i);
            printf("\tReply from : %s ,time : %g ms\n",target,dt);
	        done = 1;
	        break;  // Break out of Receive loop.
